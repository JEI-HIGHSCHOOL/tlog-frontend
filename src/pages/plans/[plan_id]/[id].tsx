...
import Slider from '@/components/layout/ImageSlider';
...
export const getServerSideProps: GetServerSideProps<ServerSideProps> = async (
  ...
  if (data.status == 401) {
    return {
      ...
      redirect: {
        permanent: false,
        destination: `/login?redirect=/plans/${encodeURI(
          context.params?.plan_id as string
        )}/${encodeURI(context.params?.id as string)}`,
      },
    };
        ...
        render={({ state, fullpageApi }) => {
          return (
            <ReactFullpage.Wrapper>
              <section
                className='section'
                style={{ fontFamily: 'LeeSeoyun', width: '100vw' }}
              >
                <div className='overflow-hidden'>
                  {plan?.planImage?.length === 0 || image.length === 0 ? (
                    <>
